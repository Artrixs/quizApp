...
class CheckBoxQuestion extends AbstractQuestion
{
    protected $options;
    ...
    public function getHTML(){
        ...
        for( $i = 0; $i < count($this->options) - 1; $i++){
            ...
            $result .= "\t" . "<input type=\"checkbox\" name=\"{$this->number}\" value=\"$option\">$option<br>" . \PHP_EOL;
        }

        $option = $this->options[count($this->options)-1];
        $result .= "\t" . "<input type=\"checkbox\" name=\"{$this->number}\" value=\"$option\">$option" . \PHP_EOL;
